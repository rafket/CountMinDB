...
int main(int argc, char** argv) {
    if(argc!=5) {
        printf("Usage:\n    %s [number of elements] [number of updates1] [number of updates2] [number of queries]\n", argv[0]);
        exit(0);
    }
    mt19937_64 mt(1337);
    ...
    CountMin cm_tree1(epsilon, delta, 1337, HashTable, u1);
    CountMin cm_tree2(epsilon, delta, 1337, HashTable, u2);
    CountMin cm_rawlog1(epsilon, delta, 1337, RawLog, u1);
    CountMin cm_rawlog2(epsilon, delta, 1337, RawLog, u2);
    ...
    start=clock();
    for(int i=0; i<q; ++i) {
        sum4 += cm_hashtable1.innerProductQuery(cm_rawlog2);
    }
    finish=clock();
    ...
    start=clock();
    for(int i=0; i<q; ++i) {
        sum5 += cm_tree1.innerProductQuery(cm_rawlog2);
    }
    finish=clock();
    ...
    start=clock();
    for(int i=0; i<q; ++i) {
        sum6 += cm_optimized1.innerProductQuery(cm_rawlog2);
    }
    finish=clock();
